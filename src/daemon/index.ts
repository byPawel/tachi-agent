 ...
 *
 * Lifecycle:
 *   - Build the runtime ONCE; mount a SessionStore + periodic TTL/GC sweep.
 *   - On SIGINT/SIGTERM: DRAIN — reject new runs (503), broadcast a final
 *     `event: shutdown` SSE frame to every live sink, let in-flight runs finish,
 *     then hard-timeout and exit so a hung run can't wedge shutdown.
 *
 * Config (env):
 *   GATEWAY_TOKEN / GATEWAY_TOKENS  auth (required — refuse to start without it)
 *   TACHI_DAEMON_PORT               listen port (default 8787)
 *   TACHI_SESSION_TTL_MS            idle TTL before an unattached, finished session is GC'd (default 600000)
 *   TACHI_SESSION_BUFFER_MAX        per-session event ring-buffer cap (default 10000)
 */
...
async function main(): Promise<void> {
  ...
  const sessions = new SessionStore({ bufferMax: sessionBufferMax });
  ...
  // Periodic GC: evict unattached, finished sessions idle past the TTL.
  const gcEvery = Math.max(1_000, Math.min(ttlMs, 60_000)); // sweep at most once/min, at least once/sec
  const gcTimer = setInterval(() => {
    const evicted = sessions.collect(ttlMs);
    if (evicted.length) console.error(`[daemon] GC evicted ${evicted.length} idle session(s)`);
  }, gcEvery);
  ...
  const drain = async (sig: string): Promise<void> => {
    ...
    // Wait for in-flight runs (their SSE sinks) to clear, bounded by a hard timeout.
