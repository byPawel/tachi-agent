...
export interface GatewayOptions {
  /** Default ReAct cap per run (a request may lower it). */
...
export function createGatewayServer(
  ...
  return http.createServer(async (req, res) => {
    ...
    if (req.method === "POST" && parts.length === 1 && parts[0] === "runs") {
      ...
      runtime
        .orchestrator({
          maxIterations: typeof body.maxIterations === "number" ? body.maxIterations : opts.maxIterations,
    ...
    if (parts[0] === "runs" && parts[1]) {
      ...
      if (req.method === "GET" && parts.length === 3 && parts[2] === "events") {
        ...
        const unsub = registry.subscribe(record.id, (e, i) => {
          ...
          if (e.type === "final" || e.type === "error") { clearInterval(heartbeat); unsub(); res.end(); }
        });
